...
for (let i = 0; i < pages.length; i++) {
  const page = pages[i];
  console.log(`\n${"=".repeat(60)}`);
  console.log(`==> Page ${i + 1}/${pages.length}: ${page.url}`);
  console.log(`${"=".repeat(60)}`);

  console.log(`\n==> [1/4] Screenshot: ${page.url}`);
  await screenshot(page.url);

  console.log("\n==> [2/4] Split sections");
  const numSections = await split(page.url, sectionCount);

  console.log("\n==> [3/4] Component detection — hand off to Claude Code agent");
  const handoffMsg = `analyze ${numSections} sections for ${siteSlug(page.url)} page ${page.url} and write components.json`;
  console.log(`CLAUDE_AGENT_HANDOFF: ${handoffMsg}`);

  handoffs.push({ page: page.url, numSections });
}
